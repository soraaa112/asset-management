	...
	if (isset($_POST['btnCari'])) {
		if (trim($_POST['cmbTahun']) == "Semua") {
			if (trim($_POST['cmbBulan']) == "Semua") {
				...
				if (trim($_POST['cmbKategori']) == "Semua") {
					//Query #1 (all)
					$filterSQL   = "WHERE month(mutasi.tgl_mutasi) = '$cmbBulan'";
				} else {
					//Query #2 (filter)
					$filterSQL   = "WHERE month(mutasi.tgl_mutasi) = '$cmbBulan' AND barang.kd_kategori ='$kodeKategori'";
				}
			}
		} else {
			if (trim($_POST['cmbBulan']) == "Semua") {
				if (trim($_POST['cmbKategori']) == "Semua") {
					//Query #1 (all)
					$filterSQL   = "WHERE year(mutasi.tgl_mutasi) = '$cmbTahun'";
				} else {
					//Query #2 (filter)
					$filterSQL   = "WHERE year(mutasi.tgl_mutasi) = '$cmbTahun' AND barang.kd_kategori ='$kodeKategori'";
				}
			} else {
				if (trim($_POST['cmbKategori']) == "Semua") {
					//Query #1 (all)
					$filterSQL   = "WHERE year(mutasi.tgl_mutasi) = '$cmbTahun' AND month(mutasi.tgl_mutasi) = '$cmbBulan'";
				} else {
					//Query #2 (filter)
					$filterSQL   = "WHERE year(mutasi.tgl_mutasi) = '$cmbTahun' AND month(mutasi.tgl_mutasi) = '$cmbBulan' AND barang.kd_kategori ='$kodeKategori'";
				}
			}
		}
	} else {
