<body>
	...
	if (isset($_POST['btnSimpan'])) {
		...
		if (count($pesanError) >= 1) {
			...
			$mySql	= "UPDATE pengadaan SET nomor_resi = '$txtNomorResi', status = '$cmbStatus', foto = '$fileName', foto_form = '$fileForm' WHERE no_pengadaan = '$Kode'";
			...
			if ($myQry) {
				...
				$sqlQuery 	= "SELECT * FROM pengadaan WHERE no_pengadaan = '$Kode'";
				$checkQry 	= mysql_query($sqlQuery, $koneksidb) or die("Gagal Query Item : " . mysql_error());
				$checkData 	= mysql_fetch_array($checkQry);

				if ($checkData['status'] == "Sudah Diterima") {
					$Sql = "SELECT * FROM pengadaan_item WHERE no_pengadaan= '$Kode'";
					$cekSql = mysql_query($Sql, $koneksidb) or die("Query no pengadaan tidak dikenal : " . mysql_error());
					while ($dataCek = mysql_fetch_array($cekSql)) {
						$dataKode = $dataCek['kd_barang'];
						$kodeBaru = $dataCek['no_pengadaan'];
						$dataJumlah = $dataCek['jumlah'];
						...
						for ($i = 1; $i <= $dataJumlah; $i++) {
							...
							$kodeDepan		= $dataKode . ".";
							$kodeInventaris = buatKodeKoleksi2($koneksidb, "barang_inventaris", $kodeDepan);
							...
							VALUES('$kodeInventaris', '$dataKode', '$kodeBaru', '$txtTanggal', 'Tersedia')";
		...
		<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" enctype="multipart/form-data">
			<table width="900" cellspacing="1" class="table-list" style="margin-top:0px;">
				...
				<tr>
					...
					<td><strong>:</strong></td>
					<td><input type="text" name="txtTanggal" class="tcal" value="<?php echo IndonesiaTgl($tglTransaksi); ?>" size="25" maxlength="20" readonly /></td>
				</tr>
				<tr>
					<td><strong>Jenis Pengadaan </strong></td>
					<td><strong>:</strong></td>
					<td><b>
							<select name="cmbJenis" data-live-search="true" class="selectpicker show-tick">
				...
				<tr>
					...
					<td><strong>:</strong></td>
					<td><input name="txtKeterangan" value="<?php echo $dataKeterangan; ?>" size="25" maxlength="100" autocomplete="off" readonly /></td>
				</tr>
				<tr>
					<td><strong>Departemen </strong></td>
					<td><strong>:</strong></td>
					<td>
						<select name="cmbDepartemen" onchange="javascript:submitform();" data-live-search="true" class="selectpicker show-tick">
							<?php
				...
				<tr>
					...
					<td><strong>:</strong></td>
					<td><b>
							<select name="cmbLokasi" data-live-search="true" class="selectpicker show-tick">
								<?php
				...
				<tr>
					...
					<td><strong>:</strong></td>
					<td>
						<select name="statusBarang" data-live-search="true" class="selectpicker show-tick">
							<option value=""> Pilih Status </option>
							<?php
							include_once "library/inc.pilihan.php";
							foreach ($statusPengajuan as $nilai) {
			...
			<table class="table-list" width="900" border="0" cellspacing="1" cellpadding="2">
				...
				?>
					<tr bgcolor="<?php echo $warna; ?>">
						...
						<td align="center" bgcolor="#FFFFCC"><a href="index.php?open=Pengadaan-Approve-Edit&Kode=<?php echo $Kode ?>&Act=Delete&ID=<?php echo $ID; ?>" onclick="return confirm('ANDA YAKIN AKAN MENGHAPUS DATA INI ... ?')" type="button" class="btn btn-danger btn-sm" target="_self"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
					</tr>
				<?php
				} ?>
				<tr>
