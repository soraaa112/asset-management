...
$pageSql = "SELECT * FROM barang_inventaris BI
LEFT JOIN barang B ON BI.kd_barang = B.kd_barang
LEFT JOIN kategori K ON B.kd_kategori = K.kd_kategori
WHERE BI.status_aktif != 'No' $filter ORDER BY BI.kd_inventaris ASC";
			...
			<table id="example1" class="table-list" width="100%" border="0" cellspacing="1" cellpadding="2">
				...
				<tbody>
					...
					$mySql = "SELECT z.kd_inventaris, z.serial_number, z.kd_barang, z.nm_barang, z.status_barang, z.nm_petugas, z.nm_departemen, z.keterangan, z.nm_lokasi FROM
						...
						WHERE BI.status_barang = 'Tersedia' AND BI.status_aktif != 'No' $filter GROUP BY BI.kd_inventaris
						UNION
						...
						WHERE PI.status_aktif = 'No' AND BI.status_aktif != 'No' $filter GROUP BY BI.kd_inventaris
						UNION
						...
						WHERE PJ.status_kembali = 'Pinjam' AND BI.status_aktif != 'No' $filter GROUP BY BI.kd_inventaris) z GROUP BY z.kd_inventaris ORDER BY z.kd_inventaris ASC";
						...
						<tr bgcolor="<?php echo $warna; ?>">
							...
							<td bgcolor="<?php echo $warna; ?>">
								<a href="#" onClick="
									...
									window.opener.document.getElementById('serial_number').value = '<?php echo $myData['serial_number']; ?>';
